...
```mermaid
flowchart TD
    A[Meeting Input] --> B[Recorder Agent]
    B --> B1[Topics / Discussion Points]
    B --> C[Decision Agent]
    C --> D[Task Planner Agent]
    C --> E[Owner Agent]
    C --> F[Deadline Agent]
    D --> E
    E --> F
    ...
    G --> G2[Risk Flags]
    G --> G3[Conflicts]
    G --> G4[Follow-up Questions]
    G --> H[Pipeline Result]
    H --> I[HTTP API / Web UI]
    H --> J[Markdown Report]
    H --> K[Sync Agent Layer]
    K --> L[Jira]
    K --> M[Notion]
    N[LLM Hybrid Extractor] --> C
    N --> D
```
